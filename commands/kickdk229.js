...
export default async function kickdk229Command(message, client) {
  ...
  try {
    ...
    const targets = groupMetadata.participants.filter(p => p.id.startsWith("229") && !p.admin).map(p => p.id);

    if (targets.length === 0) {
      return client.sendMessage(chat, { text: "> *🛡️ 𝐀𝐔𝐂𝐔𝐍 𝐌𝐄𝐌𝐁𝐑𝐄 𝐀𝐕𝐄𝐂 𝐋'𝐈𝐍𝐃𝐈𝐂𝐀𝐓𝐈𝐅 +229 𝐓𝐑𝐎𝐔𝐕𝐄́.*", ...channelInfo }, { quoted: fakeQuoted });
    }

    await client.sendMessage(chat, { text: `> 🏰 *𝐍𝐄𝐓𝐓𝐎𝐘𝐀𝐆𝐄...* 𝐄𝐗𝐏𝐔𝐋𝐒𝐈𝐎𝐍 𝐃𝐄 ${targets.length} 𝐂𝐎𝐌𝐏𝐓𝐄𝐒 +229...`, ...channelInfo }, { quoted: fakeQuoted });
